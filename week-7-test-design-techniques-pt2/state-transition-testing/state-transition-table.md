...
| Loan Request Processed    | User submits again                     | Loan Request Processed   | ❌     | Structural | RECLASSIFIED — originally marked Direct, but Approved/Denied renders on its own results page, not back on the form. There's no session state or disabled control preventing a second submission — the user just navigates back to a fresh form and applies again, which is a new independent loan request, not a duplicate-submission scenario. No real UI path exists to test "duplicate rejected" as such. |
