...
def test_checkout_with_items(add_backpack_and_bike_light_to_cart):
    ...
    # check that the checkout page is displayed
    expect(add_backpack_and_bike_light_to_cart.locator(".title")).to_have_text("Checkout: Your Information")
    # fill in the checkout information
    ...
    add_backpack_and_bike_light_to_cart.locator("[data-test='postalCode']").fill("12345")
    # click on the 'Continue' button
    add_backpack_and_bike_light_to_cart.locator("[data-test='continue']").click()
    # check that the checkout overview page is displayed
    expect(add_backpack_and_bike_light_to_cart.locator(".title")).to_have_text("Checkout: Overview")
    ...
    expect(add_backpack_and_bike_light_to_cart.locator(".inventory_item_name").nth(0)).to_have_text("Sauce Labs Backpack")
    expect(add_backpack_and_bike_light_to_cart.locator(".inventory_item_name").nth(1)).to_have_text("Sauce Labs Bike Light")
    # check item total and tax calculations
    item_total = add_backpack_and_bike_light_to_cart.locator(".summary_subtotal_label").text_content()
    tax = add_backpack_and_bike_light_to_cart.locator(".summary_tax_label").text_content()
    total = add_backpack_and_bike_light_to_cart.locator(".summary_total_label").text_content()
    ...
    # check that the checkout complete page is displayed
    expect(add_backpack_and_bike_light_to_cart.locator(".title")).to_have_text("Checkout: Complete!")