...
def test_get_user():
    ...
    ), f"Expected 'Leanne Graham' but got {user['name']}"
    assert user["username"] == "Bret", f"Expected 'Bret' but got {user['username']}"
    assert (
        user["email"] == "Sincere@april.biz"
    ), f"Expected 'Sincere@april.biz', but got {user['email']}"
    assert (
        user["address"]["street"] == "Kulas Light"
    ), f"Expected 'Kulas Light', but got {user['address']['street']}"
    assert (
        user["address"]["city"] == "Gwenborough"
    ), f"Expected 'Gwenborough', but got {user['address']['city']}"
    assert (
        user["address"]["zipcode"] == "92998-3874"
    ), f"Expected '92998-3874', but got {user['address']['zipcode']}"
    assert (
        user["phone"] == "1-770-736-8031 x56442"
    ), f"Expected '1-770-736-8031 x56442', but got {user['phone']}"
    assert (
        user["website"] == "hildegard.org"
    ), f"Expected 'hildegard.org', but got {user['website']}"
    assert (
        user["company"]["name"] == "Romaguera-Crona"
    ), f"Expected 'Romaguera-Crona', but got {user['company']['name']}"
...
def test_create_user():
    ...
    ), f"Expected 'john123' but got {user['username']}"
    assert (
        user["email"] == "johndoe123@somebusiness.com"
    ), f"Expected 'johndoe123@somebusiness.com', but got {user['email']}"
    assert (
        user["address"]["street"] == "Sample Street"
    ), f"Expected 'Sample Street', but got {user['address']['street']}"
    assert (
        user["address"]["city"] == "Sample City"
    ), f"Expected 'Sample City', but got {user['address']['city']}"
    assert (
        user["address"]["zipcode"] == "123456"
    ), f"Expected '123456', but got {user['address']['zipcode']}"
    assert (
        user["phone"] == "12345678910"
    ), f"Expected '12345678910', but got {user['phone']}"
    assert (
        user["website"] == "john.com"
    ), f"Expected 'john.com', but got {user['website']}"
    assert (
        user["company"]["name"] == "Some Business"
    ), f"Expected 'Some Business', but got {user['company']['name']}"
...
def test_put_replaces_user():
    ...
    ), f"Expected 'jane123' but got {user['username']}"
    assert (
        user["email"] == "janedoe123@somebusiness.com"
    ), f"Expected 'janedoe123@somebusiness.com', but got {user['email']}"
    assert (
        user["address"]["street"] == "Sample Street"
    ), f"Expected 'Sample Street', but got {user['address']['street']}"
    assert (
    ...
    assert (
        user["phone"] == "10987654321"
    ), f"Expected '10987654321', but got {user['phone']}"
    assert (
        user["website"] == "jane.com"
    ), f"Expected 'jane.com', but got {user['website']}"
    assert (
        user["company"]["name"] == "Some Business"
    ), f"Expected 'Some Business', but got {user['company']['name']}"
...
def test_patching_user():
    ...
    assert (
        user["username"] == "Antonette"
    ), f"Expected 'Antonette' but got {user['username']}"
    assert (
        user["email"] == "eva123@melissa.tv"
    ), f"Expected 'eva123@melissa.tv', but got {user['email']}"
    assert (
        user["address"]["street"] == "Victor Plains"
    ), f"Expected 'Victor Plains', but got {user['address']['street']}"
    assert (
        user["address"]["city"] == "Wisokyburgh"
    ), f"Expected 'Wisokyburgh', but got {user['address']['city']}"
    assert (
        user["address"]["zipcode"] == "90566-7771"
    ), f"Expected '90566-7771', but got {user['address']['zipcode']}"
    assert (
        user["phone"] == "010-692-6593 x09125"
    ), f"Expected '010-692-6593 x09125', but got {user['phone']}"
    assert (
        user["website"] == "anastasia.net"
    ), f"Expected 'anastasia.net', but got {user['website']}"
    assert (
        user["company"]["name"] == "Deckow-Crist"
    ), f"Expected 'Deckow-Crist', but got {user['company']['name']}"
